    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            ...
            if len(self.recog_transcript) > 0: # 音声認識結果がある場合だけ処理。
                ...
                self.speak(self.cmd_all[no], 2)
                rospy.loginfo("order[%d]:%s",no, self.cmd_all[no])

                if self.cmd_all[no] == "go":
                    self.set_vel(self.LINEAR_VEL,  0)
                    self.move()
                elif self.cmd_all[no] == "stop":
                    self.set_vel(0, 0)
                    self.move()
                elif self.cmd_all[no] == "back":
                    self.set_vel(-self.Linear_vel, 0)
                    self.move()
                elif self.cmd_all[no] == "right":
                    self.set_vel(0, -self.ANGULAR_VEL)
                    self.move()
                elif self.cmd_all[no] == "left":
                    self.set_vel(0, self.ANGULAR_VEL)
                    self.move()
                else:
